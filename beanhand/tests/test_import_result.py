...
from beanhand.client.config import BeancountConfiguration
...
def _make_config(
    ...
    The main ledger carries the ``open`` directives (with ``beanhand-include:
...
class TestInitCalls:
    ...
    def test_passes_account_list(self, bc: BeancountConfiguration) -> None:
        ...
        assert args[1].data == SAMPLE_RECEIPT_DATA
        accounts_arg: list[dict[str, str]] = args[2]
...
class TestCommitRefusesModifiedIngestionFile:
    def test_commit_exits_if_ingestion_file_edited_after_read(
        ...
        # Simulate the user editing the ledger while beanhand processed the receipt.
        ingest.write_text(
