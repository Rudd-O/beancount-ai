...
backed by a tmp_path ledger, with accounts marked via beanhand-include metadata
...
from beanhand.client.beancount_loader import (  # type: ignore
    AccountRef,
    accounts_for_prompt,
    load_live_accounts,
)
...
class TestMarkerPlacement:
    def test_included_account_include_self(self, tmp_path: pathlib.Path) -> None:
        """An account with beanhand-include: "yes" on its own open is included;
    ...
    def test_account_closed_does_not_include_self_but_subaccounts_opened_are_included(
        ...
        """An account with beanhand-include: "recursively" then closed is not included;
...
class TestRules:
    def test_rules_are_per_account_and_do_not_inherit(
        ...
        main = _config_for(
            "2020-01-01 open Expenses:Food\n"
            '  beanhand-include: "recursively"\n'
            '  beanhand-rules: "parent rule"\n'
            "2020-01-01 open Expenses:Food:Groceries\n"
            '  beanhand-rules: "child rule"\n'
    ...
    def test_account_closed_does_not_impact_future_subaccount_rules(
        ...
        """An account with beanhand-include: "recursively" then closed is not included;
        but children which are open are pulled in, and their beanhand-rules are respected."""
        main = _config_for(
            ...
            '  beanhand-rules: "Female clothing goes here."\n'
            "2019-01-01 open Expenses:Current:Family:Wife:Cosmetics\n"
            '  beanhand-rules: "Cosmetics go here, including hyaluron."\n'
            "2019-01-01 open Expenses:Current:Family:Wife:Groceries\n"
            '  beanhand-rules: "Always assign to this when you see line items mentioning Living salad trio, other leafy vegetables, halbrahm (skim) milk / milchdrink / m-drink, creme fraiche, Lamate, Candida or Meridol toothpaste, Toast Vollkorn, salads / salad dressings, or Gruyere / Tilsiter cheese slices and packaged too."\n'
            "2019-01-01 open Expenses:Current:Family:Wife:Hygiene\n"
            '  beanhand-rules: "Tampons, pads, linsenmittel (contact lens fluid), and other hygiene products used by women."\n'
            "2019-01-01 open Expenses:Current:Family:Wife:Snacks\n"
            '  beanhand-rules: "Use this when you spot coffee shop / confiserie receipts as the wife loves coffee shops / confiseries. Also Brezelkönig. Also use when buying various types of coffee in grocery stores, or mint chocolate ice cream, or Signature La-Pistachio stuff, or Eve bottled drinks (e.g. lychee-flavored), flavored or plain (non-sparkling) water, mate drinks, or anything matcha-related."\n',
            tmp_path,
        )
        refs = load_live_accounts(main, AS_OF)
        byname = {r["name"]: r.get("rule") for r in refs}
...
class TestExclude:
    ...
    def test_explicit_include_overrides_recursive_exclude(
        ...
        """An account with its own explicit beanhand-include under an
        ancestor's beanhand-exclude:"recursively" is re-included."""
        main = _config_for(
...
class TestValidation:
    @pytest.mark.parametrize(
        "key, bad_value, match",
        [
            ("beanhand-include", '"yeppers"', "'yeppers'"),
            ("beanhand-include", '"all"', "'all'"),
            ("beanhand-include", '"recursive "', "'recursive '"),
            ("beanhand-include", "42", "42"),
            ("beanhand-exclude", '"nope"', "'nope'"),
            ("beanhand-exclude", "42", "42"),
        ],
    )
    ...
    def test_numeric_beanhand_include(self, tmp_path: pathlib.Path) -> None:
        main = _config_for(
            "2020-01-01 open Expenses:Food\n  beanhand-include: 42\n", tmp_path
        )
        with pytest.raises(ValueError, match="invalid beanhand-include"):
            load_live_accounts(main, AS_OF)

    def test_numeric_beanhand_rules(self, tmp_path: pathlib.Path) -> None:
        main = _config_for(
            "2020-01-01 open Expenses:Food\n  beanhand-rules: 42\n", tmp_path
        )
...
class TestConfigurationLoadLegacyKey:
    ...
    def test_raises_when_key_present(self, tmp_path: pathlib.Path) -> None:
        ...
        fp = _write_client_config(
            ...
            {
                "main_file": str(main),
                "account_list_file": str(tmp_path / "beanhand.accounts"),
            },
        )
