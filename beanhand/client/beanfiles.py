...
class FileModifiedError(Exception):
    """A Beancount file was modified (or removed) after beanhand read it.

    Raised by :meth:`FileGuard.verify` when the on-disk content no longer
    matches the snapshot taken when the file was read, so that beanhand can
...
class FileGuard:
    ...
    moment the guard was taken — right after beanhand read the file.  Before
    ...
    def verify(self) -> None:
        ...
        try:
            ...
            raise FileModifiedError(
                f"{self._path} was deleted after beanhand read it; refusing to write"
            ) from None
        if current != self._hash:
            raise FileModifiedError(
                f"{self._path} appears to have been modified since beanhand read it; "
