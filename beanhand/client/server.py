...
from beanhand.structs import AccountRef, FetchedReceipt, load_json
...
class RemoteVM:
    ...
    def _call(
        ...
        # Local fallback for testing: when target_vm is None, invoke beanhand-server directly.
        if self.target_vm is None:
            cmd = ["beanhand-server", "--config", str(Configuration.cfg_path)]
            if arg is not None:
    ...
    def help_associate_receipt(
        ...
        """
        cmd, proc, stdin, stdout = self._call(
            "beanhand.HelpAssociateReceipt", arg=filename
        )
    ...
    def process_receipt(
        ...
        and the main payment account.
        """
        cmd, proc, stdin, stdout = self._call("beanhand.Process", arg=filename)
